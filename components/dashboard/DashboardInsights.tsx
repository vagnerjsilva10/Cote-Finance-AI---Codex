  ...
  return (
    <section className={cn(DASHBOARD_CARD_SHELL_CLASSNAME, 'min-h-[320px] space-y-4')}>
      <div className="space-y-1">
        <h3 className="text-lg font-bold tracking-tight text-[var(--text-primary)]">Insights do mês</h3>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
        {loading ? (
          <>
            <DashboardSkeletonLine className="h-[120px] w-full rounded-xl" />
            <DashboardSkeletonLine className="h-[120px] w-full rounded-xl" />
          </>
        ) : (
          <>
            {primaryCards.map((card) => (
              <article
                ...
                className={cn(
                  DASHBOARD_CARD_PANEL_CLASSNAME,
                  'space-y-2 p-3 transition-all duration-200 hover:-translate-y-[2px] hover:shadow-[0_12px_26px_rgba(0,0,0,0.28)]',
                  getInsightToneClassName(card.tone)
                )}
              >
                <p className="inline-flex w-fit items-center rounded-full border border-white/10 bg-[rgba(8,15,27,0.55)] px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.18em] text-[var(--text-muted)]">
                  {card.badge}
                </p>
                <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">{card.title}</p>
                ...
                <p className="text-[11px] font-semibold text-[var(--primary)]">{card.action}</p>
              </article>
            ))}

            {currentPlan === 'FREE' ? (
              <div className={cn(DASHBOARD_CARD_PANEL_CLASSNAME, 'space-y-2.5 p-3 sm:col-span-2')}>
                <p className="inline-flex w-fit items-center rounded-full border border-white/10 bg-[rgba(8,15,27,0.55)] px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.18em] text-[var(--text-muted)]">
                  Oportunidade
                </p>
                ...
                <button
                  type="button"
                  onClick={onUpgrade}
                  className="app-button-primary rounded-lg px-3 py-1.5 text-xs font-semibold"
                >
              ...
              automatedInsights.map((insight, index) => (
                ...
                >
                  ...
                  <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">Leitura principal</p>
                  ...
                  <p className="text-[11px] font-semibold text-[var(--primary)]">{getInsightActionHint(insight)}</p>
                </article>
              ))
            )}

            {!primaryCards.length && currentPlan !== 'FREE' && !automatedInsights.length ? (
              <div className={cn(DASHBOARD_CARD_PANEL_CLASSNAME, 'p-3 sm:col-span-2')}>
                <p className="text-xs text-[var(--text-secondary)]">
