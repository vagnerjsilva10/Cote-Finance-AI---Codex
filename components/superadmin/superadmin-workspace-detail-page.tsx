    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
          ...
          <div className="space-y-3">{workspace.members.map((member) => <div key={member.id} className="rounded-2xl border border-slate-800 bg-slate-950/55 px-4 py-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{member.name || member.email}</p><p className="truncate text-xs text-slate-400">{member.email}</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-300">{member.role}</span><button type="button" disabled={isSaving || member.role === 'MEMBER'} onClick={async () => { try { setIsSaving(true); setError(null); setActionMessage(null); const nextRole = member.role === 'OWNER' ? 'ADMIN' : 'MEMBER'; await fetchSuperadminJson(`/api/superadmin/workspaces/${workspace.id}`, { method: 'PATCH', body: JSON.stringify({ action: 'update-member-role', memberId: member.id, memberRole: nextRole }) }); setData((current) => current ? { ...current, workspace: { ...current.workspace, members: current.workspace.members.map((item) => item.id === member.id ? { ...item, role: nextRole } : item) } } : current); setActionMessage('Papel do membro atualizado com sucesso.'); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Falha ao atualizar papel do membro.'); } finally { setIsSaving(false); } }} className={secondaryActionClassName}>Rebaixar</button><button type="button" disabled={isSaving || member.role === 'OWNER'} onClick={async () => { try { setIsSaving(true); setError(null); setActionMessage(null); const nextRole = member.role === 'MEMBER' ? 'ADMIN' : 'OWNER'; const response = await fetchSuperadminJson<{ ok: boolean; member: { id: string; role: string } }>(`/api/superadmin/workspaces/${workspace.id}`, { method: 'PATCH', body: JSON.stringify({ action: 'update-member-role', memberId: member.id, memberRole: nextRole }) }); setData((current) => current ? { ...current, workspace: { ...current.workspace, owner: nextRole === 'OWNER' ? { userId: member.userId, name: member.name, email: member.email } : current.workspace.owner, members: current.workspace.members.map((item) => ({ ...item, role: item.id === response.member.id ? response.member.role : nextRole === 'OWNER' && item.role === 'OWNER' ? 'ADMIN' : item.role })) } } : current); setActionMessage('Papel do membro atualizado com sucesso.'); } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Falha ao atualizar papel do membro.'); } finally { setIsSaving(false); } }} className={primaryActionClassName}>{member.role === 'MEMBER' ? 'Promover' : 'Tornar owner'}</button></div></div></div>)}</div>
        </section>
