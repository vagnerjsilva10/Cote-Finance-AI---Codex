...
import { cancelFinancialEvent, syncWorkspaceFinancialCalendarSourcesSafe } from '@/lib/server/financial-calendar';
import {
...
export async function POST(req: Request, { params }: { params: Promise<{ eventId: string }> }) {
  try {
    ...
    const result = await cancelFinancialEvent({
    ...
    await logWorkspaceEventSafe({
      workspaceId: context.workspaceId,
      userId: context.userId,
      type: 'financial_calendar.event.canceled',
      payload: {
    ...
    return buildFinancialCalendarErrorResponse(error, 'Financial calendar event cancel Error:');
  }
}
