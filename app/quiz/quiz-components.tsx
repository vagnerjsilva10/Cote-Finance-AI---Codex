...
export function QuizContainer({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <main className={`min-h-dvh overflow-x-clip bg-slate-950 text-slate-100 ${className}`}>
...
export function QuestionCard({
  ...
  return (
    <motion.section
      ...
      className="rounded-[1.5rem] border border-white/10 bg-slate-900/65 p-4 shadow-[0_24px_80px_-36px_rgba(15,23,42,.8)] sm:rounded-[2rem] sm:p-8"
    >
      {eyebrow ? (
        <span className="inline-flex max-w-full rounded-full border border-emerald-300/30 bg-emerald-500/10 px-3 py-1 text-xs font-semibold leading-5 text-emerald-200">
          {eyebrow}
        </span>
      ) : null}
      <h1 className="mt-4 text-[1.65rem] font-bold leading-tight text-white sm:text-4xl">{title}</h1>
...
export function AnswerButton({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`flex min-h-[72px] w-full items-center justify-between rounded-2xl border px-4 py-4 text-left text-[15px] font-medium leading-6 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/70 sm:text-base ${
        selected
  ...
  return (
    <QuestionCard eyebrow="Diagnóstico em andamento" title={title}>
      <div className="space-y-6">
        {statusLabel ? (
          <div className="inline-flex max-w-full rounded-full border border-cyan-300/25 bg-cyan-500/10 px-3 py-1 text-xs font-semibold leading-5 text-cyan-100">
            {statusLabel}
          </div>
        ) : null}
        <p className="text-sm leading-7 text-slate-300 sm:text-base">{text}</p>
